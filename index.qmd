...
ax.scatter(dreamy_data['patient'], dreamy_data['post_surgical_score'], 
           marker='o', color='#4E79A7', s=80, alpha=0.7, 
           label='Doc Dreamy', edgecolors='white', linewidths=1.5, zorder=3)

# Doc Duck: triangles, coral/red-orange
ax.scatter(duck_data['patient'], duck_data['post_surgical_score'], 
           marker='^', color='#E15759', s=80, alpha=0.7, 
           label='Doc Duck', edgecolors='white', linewidths=1.5, zorder=3)
...
# Doc Dreamy annotation with ○ symbol
ax.text(102, dreamy_mean, f'○ Mean = {dreamy_mean:.2f}', 
        fontsize=11, color='#4E79A7', weight='bold', 
        verticalalignment='center', zorder=4)

# Doc Duck annotation with △ symbol
ax.text(102, duck_mean, f'△ Mean = {duck_mean:.2f}', 
        fontsize=11, color='#E15759', weight='bold', 
        verticalalignment='center', zorder=4)

# Professional styling
ax.set_xlabel('Patient Number', fontsize=13, weight='bold')
...
# Filter randomized data by doctor
...
# Filter observational data by doctor
dreamy_data = patients_df[patients_df['doctor_name'] == 'Doc Dreamy']
duck_data = patients_df[patients_df['doctor_name'] == 'Doc Duck']
...
# Calculate mean severity for each doctor (to show confounding)
dreamy_mean_severity = dreamy_data['severity'].mean()
duck_mean_severity = duck_data['severity'].mean()

# Calculate means within stratification region (-1 to 1)
stratification_mask_dreamy = (dreamy_data['severity'] >= -1) & (dreamy_data['severity'] <= 1)
stratification_mask_duck = (duck_data['severity'] >= -1) & (duck_data['severity'] <= 1)

dreamy_mean_stratified = dreamy_data[stratification_mask_dreamy]['post_surgical_score'].mean()
duck_mean_stratified = duck_data[stratification_mask_duck]['post_surgical_score'].mean()
...
# Add vertical lines showing mean severity for each doctor (reveals confounding)
ax.axvline(x=dreamy_mean_severity, color='#4E79A7', linestyle='--', 
           linewidth=2.5, alpha=0.7, zorder=1, label='Doc Dreamy Mean Severity')
ax.axvline(x=duck_mean_severity, color='#E15759', linestyle='--', 
           linewidth=2.5, alpha=0.7, zorder=1, label='Doc Duck Mean Severity')
...
# Add horizontal lines for stratified means within -1 to 1 region
ax.plot([-1, 1], [dreamy_mean_stratified, dreamy_mean_stratified], 
        color='#4E79A7', linestyle='-', linewidth=3.5, alpha=0.9, zorder=4)
ax.plot([-1, 1], [duck_mean_stratified, duck_mean_stratified], 
        color='#E15759', linestyle='-', linewidth=3.5, alpha=0.9, zorder=4)

# Add text annotations for mean severity (showing confounding in assignment)
ax.text(dreamy_mean_severity, ax.get_ylim()[1] * 0.98, 
        f'○ Mean Severity\n{dreamy_mean_severity:.2f}', 
        fontsize=10, color='#4E79A7', weight='bold',
        verticalalignment='top', horizontalalignment='center',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9, edgecolor='#4E79A7'),
        zorder=5)

ax.text(duck_mean_severity, ax.get_ylim()[1] * 0.98, 
        f'△ Mean Severity\n{duck_mean_severity:.2f}', 
        fontsize=10, color='#E15759', weight='bold',
        verticalalignment='top', horizontalalignment='center',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9, edgecolor='#E15759'),
        zorder=5)

# Stratified means (solid lines in stratification region)
ax.text(0, dreamy_mean_stratified + 0.15, f'○ Mean = {dreamy_mean_stratified:.2f}', 
        fontsize=11, color='#4E79A7', weight='bold',
        verticalalignment='center', horizontalalignment='center', 
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9, edgecolor='#4E79A7', linewidth=2),
        zorder=5)

ax.text(0, duck_mean_stratified - 0.15, f'△ Mean = {duck_mean_stratified:.2f}', 
        fontsize=11, color='#E15759', weight='bold',
        verticalalignment='center', horizontalalignment='center',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9, edgecolor='#E15759', linewidth=2),
        zorder=5)

# Add annotation showing the confounding
x_max = max(patients_df['severity'].max(), abs(patients_df['severity'].min()))
severity_diff = duck_mean_severity - dreamy_mean_severity

ax.annotate(f'Confounding: Doc Duck sees patients\n{severity_diff:.2f} severity points higher on average',
            xy=(duck_mean_severity, ax.get_ylim()[0] + 0.5),
            xytext=(x_max * 0.6, ax.get_ylim()[0] + 1.2),
            fontsize=11, weight='bold', color='#444444',
            arrowprops=dict(arrowstyle='->', color='#444444', lw=2),
            bbox=dict(boxstyle='round,pad=0.8', facecolor='#FFE6E6', alpha=0.95, edgecolor='#444444', lw=2),
            zorder=6, horizontalalignment='center')

# Add annotation for stratification region comparison
if duck_mean_stratified < dreamy_mean_stratified:
    diff = dreamy_mean_stratified - duck_mean_stratified
    ax.text(0, ax.get_ylim()[1] * 0.15, 
            f'Within overlap region (-1 to 1 severity):\nDoc Duck outperforms by {diff:.2f} points',
            fontsize=11, weight='bold', color='#E15759',
            verticalalignment='center', horizontalalignment='center',
            bbox=dict(boxstyle='round,pad=0.8', facecolor='#FFF3CD', alpha=0.95, edgecolor='#E15759', lw=2.5),
            zorder=6)

# Professional styling
ax.set_xlabel('Initial Patient Severity', fontsize=13, weight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', 
              fontsize=13, weight='bold')
ax.set_title('Revealing the Confounder: Severity Affects Both Surgeon Assignment and Outcomes', 
             fontsize=15, weight='bold', pad=20)

# Add grid
ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5, zorder=1)

# Add legend with only scatter points
handles, labels = ax.get_legend_handles_labels()
# Keep only the scatter plot entries
filtered_handles = [h for h, l in zip(handles, labels) 
                    if l in ['Doc Dreamy', 'Doc Duck']]
filtered_labels = [l for l in labels 
                   if l in ['Doc Dreamy', 'Doc Duck']]
ax.legend(filtered_handles, filtered_labels, loc='upper left', fontsize=12, 
...
Enhanced Figure 3 to show that Doc Duck performs better when controlling for patient severity. Changes:
Regression lines: Added trend lines for each doctor to show the relationship between severity and outcomes.
Stratified means: Calculated and displayed means within the stratification region (-1 to 1 severity) with dotted horizontal lines, showing performance for similar-severity patients.
Visual comparison:
Overall means (dashed lines) show the confounding effect
Stratified means (dotted lines) show the true comparison
Annotations highlight the stratified means in the overlapping region
Highlight annotation: Added an annotation stating how much better Doc Duck performs in the overlapping severity region, with an arrow pointing to the comparison.
Legend: Cleaned up to exclude trend lines and the stratification region.
The figure shows:
Overall, Doc Dreamy appears better (due to confounding)
When controlling for severity (stratification region -1 to 1), Doc Duck performs better
The regression lines show the trends, making the comparison clearer
This aligns with Figure 2 (randomized data), where Doc Duck is shown to be the better doctor. Re-render the document to see the updated figure.
