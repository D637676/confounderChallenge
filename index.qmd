...
# Fit regression lines for trend visualization
dreamy_slope, dreamy_intercept, _, _, _ = stats.linregress(
    dreamy_data['severity'], dreamy_data['post_surgical_score']
)
duck_slope, duck_intercept, _, _, _ = stats.linregress(
    duck_data['severity'], duck_data['post_surgical_score']
)
...
x_trend = np.linspace(patients_df['severity'].min(), patients_df['severity'].max(), 100)
dreamy_trend = dreamy_slope * x_trend + dreamy_intercept
duck_trend = duck_slope * x_trend + duck_intercept

ax.plot(x_trend, dreamy_trend, color='#4E79A7', linestyle='-', 
        linewidth=2.5, alpha=0.6, label='Doc Dreamy Trend', zorder=2)
ax.plot(x_trend, duck_trend, color='#E15759', linestyle='-', 
        linewidth=2.5, alpha=0.6, label='Doc Duck Trend', zorder=2)
...
ax.axhline(y=dreamy_mean_stratified, color='#4E79A7', linestyle=':', 
           linewidth=2.5, alpha=0.8, zorder=2)
ax.axhline(y=duck_mean_stratified, color='#E15759', linestyle=':', 
           linewidth=2.5, alpha=0.8, zorder=2)

# Add text annotations with mean values and shape symbols
# Position annotations on the right side of the plot
x_max = max(patients_df['severity'].max(), abs(patients_df['severity'].min()))
x_annotation = x_max * 0.95

# Overall means (dashed lines)
ax.text(x_annotation, dreamy_mean, f'○ Overall Mean = {dreamy_mean:.2f}', 
        fontsize=10, color='#4E79A7', weight='bold', style='italic',
        verticalalignment='center', horizontalalignment='right', zorder=4)

ax.text(x_annotation, duck_mean, f'△ Overall Mean = {duck_mean:.2f}', 
        fontsize=10, color='#E15759', weight='bold', style='italic',
        verticalalignment='center', horizontalalignment='right', zorder=4)

# Stratified means (dotted lines) - highlight in stratification region
ax.text(0, dreamy_mean_stratified, f'○ Stratified Mean = {dreamy_mean_stratified:.2f}', 
        fontsize=11, color='#4E79A7', weight='bold',
        verticalalignment='center', horizontalalignment='center', 
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8, edgecolor='#4E79A7'),
        zorder=5)

ax.text(0, duck_mean_stratified, f'△ Stratified Mean = {duck_mean_stratified:.2f}', 
        fontsize=11, color='#E15759', weight='bold',
        verticalalignment='center', horizontalalignment='center',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8, edgecolor='#E15759'),
        zorder=5)

# Add annotation highlighting that Doc Duck is better in stratification region
if duck_mean_stratified < dreamy_mean_stratified:
    diff = dreamy_mean_stratified - duck_mean_stratified
    ax.annotate(f'Doc Duck performs {diff:.2f} points better\nin overlapping severity region (-1 to 1)',
                xy=(0, (dreamy_mean_stratified + duck_mean_stratified) / 2),
                xytext=(x_max * 0.55, (dreamy_mean_stratified + duck_mean_stratified) / 2 + 0.6),
                ...
                zorder=6, horizontalalignment='left')
...
# Add legend (excluding the shaded region and trend lines from legend to keep it clean)
handles, labels = ax.get_legend_handles_labels()
# Filter out the shaded region and trend lines from legend
filtered_handles = [h for h, l in zip(handles, labels) 
                    if l not in ['Stratification Region', 'Doc Dreamy Trend', 'Doc Duck Trend']]
filtered_labels = [l for l in labels 
                   if l not in ['Stratification Region', 'Doc Dreamy Trend', 'Doc Duck Trend']]
ax.legend(filtered_handles, filtered_labels, loc='upper right', fontsize=12, 
